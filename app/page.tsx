...
type Modal = "hotel" | "train" | "transit" | "sos" | "install" | null;
...
export default function Home() {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    const result = places.filter((place) => {
      ...
      return matchesCategory && matchesQuiet && (!needle || haystack.includes(needle));
    });
  ...
  }, [category, position, query, quietOnly, sort]);
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <button className="brand" onClick={() => setTab("plan")} aria-label="Zur Startseite"><Image src="/app-icon.svg" alt="" width={43} height={43} priority /><span><b>Amsterdam × Zandvoort</b><small>21.–23. AUG · NL F1 WEEKEND</small></span></button>
        ...
        <div className="top-actions"><button className="icon-button" onClick={() => setModal("sos")} aria-label="Notfallinformationen">SOS</button><button className="icon-button" onClick={() => setDark((value) => !value)} aria-label={dark ? "Helles Design" : "Dunkles Design"}>{dark ? "☀" : "☾"}</button></div>
      </header>
      ...
      <div className="page-wrap">
        {tab === "plan" && <>
          ...
          <section className="urgent-card"><div className="urgent-icon">!</div><div><p className="eyebrow">VOR DER ABFAHRT</p><h2>2× Dutch GP Zugticket kaufen</h2><p>Direktzug Amsterdam Centraal → Zandvoort aan Zee · die 15-Jährige braucht ein reguläres Ticket, keinen Railrunner.</p></div><ActionLink href={ticketUrl} primary>Bei NS kaufen</ActionLink></section>
